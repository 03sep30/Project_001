...
public class Test_006 : MonoBehaviour
{
    ...
    void Start()
    {
        array[0] = 2;                                           //각 배열에 정수를 직접 입력
        array[1] = 10;                                          
        array[2] = 5;                                           
        array[3] = 15;                                           
        array[4] = 3;                                           

        for(int i = 0; i < 5; i++)                              //for문을 통해서 console 창에 모든 array 배열 안에 있는0 ~ 4까지
        {                                                       //내용을 표시
            Debug.Log(array[i]);
        }
        ...
        int sum = 0;                                            //합값을 구하기 위해서 임시적으로 sum 변수 선언

        for (int i = 0; i < points.Length; i++)                     
        {
            sum += points[i];                                   //선언된 배열의 합값
        }

        int average = sum / points.Length;                      //배열의 합값에서 배열 길이만큼 나눈다. (평균값)
